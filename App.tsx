...
import { Heart, ArrowLeft, Settings, Edit } from 'lucide-react';

const STORAGE_KEY = 'nlg_formulas_v2';
...
const App: React.FC = () => {
  ...
  // Combine title and steps for the reader
  const getReadableText = (formula: Formula) => {
    const stepsText = formula.steps.join('. ');
    return `Bài tập: ${formula.title}. ${formula.subtitle}. ${stepsText}`;
  };
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-slate-950 selection:bg-pink-500/30 selection:text-pink-100 font-inter">
      ...
      {/* Header */}
      <header className="flex-shrink-0 h-16 border-b border-white/5 bg-slate-950/80 backdrop-blur-xl flex items-center justify-between px-4 md:px-6 z-20 sticky top-0">
      ...
      {/* Main Content Area */}
      <main className="flex-grow overflow-y-auto px-4 md:px-6 py-6 scroll-smooth bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-slate-950 to-slate-950">
        <div className="max-w-4xl mx-auto min-h-full pb-20"> {/* pb-20 for bottom bar space */}
            ...
            {view === 'detail' && currentFormula && (
              <div className="animate-fade-in space-y-6">
                ...
                {/* Timer Section */}
                <div className="bg-slate-900/50 rounded-3xl border border-slate-800/50 p-1">
                   <TimerWidget 
                ...
                <div className="bg-slate-900/50 rounded-2xl p-6 md:p-8 border border-slate-800 shadow-xl relative">
                  ...
                  <div className="space-y-6">
                    <h3 className="text-lg font-semibold text-slate-200">Các bước thực hiện:</h3>
                    {currentFormula.steps.map((step, idx) => (
                      <div key={idx} className="flex gap-4">
                        ...
                        <p className="text-slate-300 leading-relaxed text-sm md:text-base pt-1 whitespace-pre-line">
                          {step}
                        </p>
                      </div>
                    ))}
      ...
      {/* Footer / Controls - Only shown in Detail View */}
      {view === 'detail' && currentFormula && (
        <div className="flex-shrink-0 z-20">
            <div className="bg-slate-950 border-t border-white/5 pb-6 pt-4 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
               <ReaderBar textToRead={getReadableText(currentFormula)} />
            </div>
        </div>
      )}
